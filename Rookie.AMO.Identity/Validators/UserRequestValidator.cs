...
namespace Rookie.AMO.Identity.Validators
{
    ...
    {
        public UserRequestValidator(UserManager<User> userManager)
        {
            ...
            RuleFor(x => x).MustAsync(
             ...
             {
                 var exist = await userManager.FindByNameAsync(Regex.Replace(dto.UserName, @"\s", ""));
                 return exist == null;
             }
            )/*.When(a => a.id == null)*/.WithMessage("Duplicate username");
